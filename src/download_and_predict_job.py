...
def download_tile(x: int, y: int, data: pd.DataFrame, api_key) -> None:
    ...
    print(f"Downloading {x, y} tile to {folder}")
...
def predict_subtile(pred_files, sess) -> np.ndarray:
    ...
    if np.sum(x) > 0:
        ...
        for i in range(len(batches)-1):
            ...
            batch_pred = sess.run(predict_logits,
                                  feed_dict={predict_inp :batch_x, 
                                             predict_length:lengths}).reshape(batch_x.shape[0], 14, 14)
            preds[batches[i]:batches[i+1]] = batch_pred

        # Converts the 14x14 predictions to a 140x140 subtile, with no shifts
        preds_stacked = []
        ...
        # Converts the 14x14 predictions to a 140x140 subtile, with a 7x7 shift
        preds_overlap = []
...
if __name__ == '__main__':
    ...
    parser.add_argument("--predict_model_path", dest = 'predict_model_path', default = '../models/master-13250/230k/')
