...
def download_tile(x: int, y: int, data: pd.DataFrame, api_key, year) -> None:
    ...
    if not (os.path.exists(clouds_file)):
        ...
        ## HERE CHECK FOR LOCAL CLOUDS, IF <= 2 DAYS WITH <= 40% CLOUD COVER,
        # MAKE SURE TO GET THE 3 LEAST LOCAL CLOUD IMAGES
...
def process_tile(x: int, y: int, data: pd.DataFrame, 
    ...
    s1 = hkl.load(s1_file)
    #s1 = s1[3:-3]
    #s1 = s1.repeat(2, axis = 0)
    #print(np.mean(s1, axis = (1, 2, 3)))
    ...
    ndsis = snow_filter(sentinel2)#ndsi(sentinel2) > 0.2
    ...
    if (len(to_remove) > 0) and args.snow:
...
def make_and_smooth_indices(arr, dates):
    ...
    #indices[np.isnan(indices)] = np.nanmedian(indices, axis = 0)[np.newaxis][np.isnan(indices)]
    try:
...
def process_subtiles(x: int, y: int, s2: np.ndarray = None, 
                       ...
                       sess = None, bbx = None, size = SIZE) -> None:
    ...
    write_ard_to_tif(s2_median[0, ..., :3], bbx, f"{str(x)}X{str(y)}Y{str(year)}", "")
    #dem_train = dem[np.newaxis, ..., np.newaxis]
    #dem_train = dem_train.repeat(12, axis = 0)
    #train_sample = np.concatenate([s2[..., :10], 
    #    dem_train, s1,
    #    s2[..., 10:]], axis = -1)
    #hkl.dump(train_sample, "train_sample.hkl", mode='w', compression='gzip')

    if args.gen_composite:
    ...
    if args.make_training_data:
        ...
        s2_train = s2[:, PX_y - 14:PX_y + 14, PX_x - 14:PX_x + 14, :]
        s1_train = s1[:, PX_y - 14:PX_y + 14, PX_x - 14:PX_x + 14, :]
        dem_train = dem[PX_y - 14:PX_y + 14, PX_x - 14:PX_x + 14]
        ...
        fout = f"train-ard/{str(PLOTID)}.npy"
        np.save(fout, train_sample)
        key = f'train-plantation-ard/{str(PLOTID)}.npy'
    ...
    if args.process is True:
        ...
        for t in tqdm(range(len(tiles_folder))):
            ...
            min_clear_images_per_date = np.sum(interp_tile < 0.67, axis = (0))
            no_images = False
            ...
            if no_images:
                ...
                if args.gen_feats:
                    ...
                    earlyfeats = predict_subtile(subtile_all, sess, predict_earlyfeats, SIZE)[..., :64]
                    #earlyfeats = earlyfeats.repeat(4, axis = 0).repeat(4, axis = 1)
                    #earlyfeats = earlyfeats[1:-1, 1:-1]
                    #print(latefeats.shape, earlyfeats.shape)
            ...
            if SIZE == 158:
                ...
                no_images = np.reshape(no_images, ((8, 20, 8, 20)))
                no_images = np.sum(no_images, axis = (1, 3))
                no_images = no_images > (20*20) * 0.75 # 0.10
                no_images = no_images.repeat(20, axis = 0).repeat(20, axis = 1)
            ...
            evi_flag = binary_dilation(1 - evi_flag, iterations = 2)
            evi_flag = binary_dilation(1 - evi_flag, iterations = 2)
            preds[np.logical_and(evi_flag, preds < 0.8)] = preds[np.logical_and(evi_flag, preds < 0.8)] ** 2.5
...
def write_ard_to_tif(arr: np.ndarray,
    ...
    print("Writing", file, np.mean(arr))
    new_dataset = rasterio.open(file,
                                ...
                                transform=transform)
    new_dataset.write(np.rollaxis(arr, 2))
    new_dataset.close()
    return file
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default = '../models/224-tml-testing/'
    )
    ...
    parser.add_argument("--redownload_s1", dest = "redownload_s1", default = False)
    ...
    parser.add_argument("--gen_feats", dest = "gen_feats", default = False)
    parser.add_argument("--gen_composite", dest = "gen_composite", default = False)
    parser.add_argument("--process", dest = "process", default = True)
    parser.add_argument("--snow", dest = "snow", default = False)
    parser.add_argument("--length", dest = "length", default = 12)
    parser.add_argument("--make_training_data", dest = "make_training_data", default = False)
    ...
    if os.path.exists(args.predict_model_path2):
        print(f"Loading model from {args.predict_model_path2}")
        predict_file2 = tf.io.gfile.GFile(args.predict_model_path2 + "predict_graph.pb", 'rb')
        predict_graph_def2.ParseFromString(predict_file2.read())
        predict_graph2 = tf.import_graph_def(predict_graph_def2, name='predict')
        predict_sess2 = tf.compat.v1.Session(graph=predict_graph2)
        if args.length == 12:
            predict_latefeats2 = predict_sess2.graph.get_tensor_by_name(f"predict/csse_out_mul/mul:0") 
            predict_earlyfeats2 = predict_sess2.graph.get_tensor_by_name(f"predict/gru_drop/drop_block2d/cond/Merge:0")
        #predict_logits = "predict/cropping2d_3/strided_slice:0" #'predict/conv2d_13/Sigmoid:0'
        #predict_height = 'predict/cropping2d_3/strided_slice:0'
        predict_logits2 = 'predict/conv2d_13/Sigmoid:0'
        print(f"predict logits: {predict_logits}")
        predict_logits2 = predict_sess2.graph.get_tensor_by_name(predict_logits2) 
        #predict_height = predict_sess.graph.get_tensor_by_name(predict_height) 
        predict_inp2 = predict_sess2.graph.get_tensor_by_name("predict/Placeholder:0")
        predict_length2 = predict_sess2.graph.get_tensor_by_name("predict/PlaceholderWithDefault:0")
    #else:
    #    raise Exception(f"The model path {args.predict_model_path2} does not exist")
    ...
    #if args.predict_model_path != "../models/224-tml-testing/":
    ...
    for index, row in data.iterrows():
        ...
        to_process = True
        if args.make_training_data:
            PX_x = row['X_px']#.astype(int)
            PX_y = row['Y_px']#.astype(int)
            print(PX_x, PX_y)
            PLOTID = row['plotid']
        ...
        if ((not processed) or args.reprocess) and (to_process == True):
            try:
                bbox = None
                if not bbx:
                    data2 = data.copy()
                ...
                if n_images > 2:
                    ...
                    time1 = time.time()
                    process_subtiles(x, y, s2, dates, interp, s1, dem, predict_sess, bbx, SIZE)
                    if args.process is True:
                    ...
                    if args.ul_flag:
                        upload_raw_processed_s3(path_to_tile, x, y, uploader, year)
                    time2 = time.time()
                    print(f"Finished {n}/{n_to_process} in {np.around(time2 - time0, 1)}"
