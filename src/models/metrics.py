...
def compute_surface_distances(mask_gt, mask_pred, spacing_mm, sort = True):
...
'''
def compute_f1_score_at_tolerance(surface_distances, tolerance_mm):
    """Computes the _surface_ DICE coefficient at a specified tolerance.
  ...
  """
    ...
    # For each item in the ground truth, how many are within tolerance to a prediction
    ...
    if true_positives < len(surfel_areas_pred):
        false_positives = len(surfel_areas_pred[distances_pred_to_gt >= tolerance_mm])
    else:
        false_positives = 0
